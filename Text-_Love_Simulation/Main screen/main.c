...
void main()
{
	...
	//#이름 나누기
	Sleep(3000); //시간 지연으로 한다.
	getchar();
	clearConsole();
	printf("==============================================\n");
	printf("[새로운 학교, 새로운 반.. 새로운 친구들.. 그리고 그렇게 정해진 나의 짝은.. 아까 부딪쳤던 그녀였다.]\n\n");
	...
	getchar();
	clearConsole();
	printf("선생님: 앞으로 짝궁과 함께 팀 활동을 할거니 그렇게 알아두렴\n\n");
	printf("1.잘 부탁해. 우리 잘 맞출 수 있을 거야.\n2.이거 빨리 끝내고 각자 할 일 하자.\n3.수업 따위 귀찮지? 우리 그냥 대충 해도 될 것 같은데.\n");
	scanf("%d", &d);
	switch (d)
	{
	case 1:
		printf("아이리: 잘 맞출 수 있을지 누가 알아. 근데… 뭐, 너라면 잘 할 수 있을지도.\n");
		love += 10;
		break;
	case 2:
		printf("아이리: 흥, 빨리 끝내고 싶어하는 건 나도 마찬가지야. 너무 귀찮게 굴지 마.\n");
		love += 0;
		break;
	case 3:
		
		printf("아이리: 대충 하다니… 넌 정말 책임감이 없네. 그러고도 잘할 수 있다고 생각해?\n");
		love -= 10;
		break;
	default:
		break;
	...
	clearConsole();
	printf("==============================================\n");
	printf("[점심시간 교실에 혼자 앉아 있는 아이리가 보였다. 나도 모르게 그녀에게 다가갔다.]\n\n");
	printf("1.혼자 있어? 같이 밥 먹을래?\n2.어디 가? 혼자 있을 때는 좀 외로워 보이네.\n3.혼자 있을 때가 편하지? 그럼, 나도 신경 쓰지 않을게\n");
	scanf("%d", &e);
	switch (e)
	{
	case 1:
		printf("아이리: (살짝 놀란 표정) 혼자 먹는 게 더 편한데… 그래도, 네가 굳이 원한다면야..\n");
		love += 10;
		break;
	case 2:
		printf("아이리: 외롭긴 누가 외롭다는 거야! 혼자 있어도 괜찮거든?\n");
		love += 5;
		break;
	case 3:

		printf("아이리: 그래, 너는 나한테 신경 안 써도 돼. 원래 혼자가 더 편해.\n");
		love -= 5;
		break;
	default:
		break;
	...
	clearConsole();
	printf("==============================================\n");
	printf("[복도를 지나가다가 아이리가 다른 친구들과 대화를 하고 있는 모습이 보인다.]\n[다른 친구들이 모두 떠났을 때 나는 아이리한테 모습을 드러냈다.]\n\n");
	printf("1.의외로 친구들이 많네. 그동안 혼자 있는 줄 알았는데.\n2.너도 친구들 앞에서는 꽤 잘 어울리네. 보기 좋다.\n3.친구들한테는 츤데레 아니네? 나한테만 그런 거지?\n");
	scanf("%d", &f);
	switch (f)
	{
	case 1:
		printf("아이리: 친구들? 뭐, 어쩌다 보니… 그렇다고 특별히 친한 건 아니야.\n");
		love += 5;
		break;
	case 2:
		printf("아이리: (살짝 당황하며) 뭐? 나랑 어울린다고? 별로 신경 쓰지 마.\n");
		love += 10;
		break;
	case 3:

		printf("아이리: 흥! 내가 너한테만 그런 건 아니야! 착각하지 마.\n");
		love -= 5;
		break;
	default:
		break;
	...
	if (love >= 70)
	{
		...
		printf("아이리: 너랑 이렇게 지내다 보니… 알게 됐어.내가 네 곁에 있으면 더 이상 혼자가 아니란 걸. 이제는 솔직해져야겠지 ?\n");
		printf("%s : 그동안 고마웠어, 그리고 앞으로도 잘 부탁해\n",name);
		printf("아이리: 정말… 나도 고마워. 너 덕분에 다시 사람을 믿을 수 있게 됐어.\n");
		printf("게임을 플레이 해주셔서 감사합니다.");
	}
	else if (love >= 31)
	{
		setColor(GRAY);
		printf("노멀 엔딩\n");
		printf("[아이리와 조금 가까워지긴 했지만, 우리는 서로에게 완전히 마음을 열지는 못했다. 우리는 그렇게 서로 애매한 감정을 남긴 채 학교생활을 마무리하였다.]\n");
		printf("아이리: 우리… 꽤 가까워지긴 했네. 하지만 여전히 뭔가 부족한 느낌이야. 그래도, 난 이제 혼자가 아니니까 괜찮아.\n");
		printf("%s : 서로 더 노력했으면 좋았을 텐데.\n",name);
		printf("아이리: (살짝 쓸쓸하게) 그래, 하지만 어쩌면 이게 우리에게 주어진 최선일지도 몰라.\n");
		printf("게임을 플레이 해주셔서 감사합니다.");
	}
	else
	{
		setColor(RED);
		printf("배드 엔딩\n");
		printf("[아이리와 나는 서로 상처를 주며 신뢰를 쌓지 못하였다. 우리는 이렇게 관계의 끝이 왔다.]\n");
		printf("아이리: 이제 와서 생각해보니… 우린 처음부터 맞지 않았던 것 같아. 너도 그렇지? 내가 너무 기대했나 봐.\n");
